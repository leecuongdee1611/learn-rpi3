...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("MinhTam-ThanhLong");
MODULE_DESCRIPTION("A gpio sysfs driver, with source code referenced from Kiran Nayak");
...
int gpio_sysfs_remove(struct platform_device *pdev)
{
	...
	for(i = 0 ; i < gpio_drv_data.total_devices ; i++){
        /*dev_info(&pdev->dev,"Remove a device\n");*/
...
int gpio_sysfs_probe(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(parent,child)
	{
		...
		dev_info(dev,"before devm_fwnode_get_gpiod_from_child \n");
		dev_data->desc = devm_fwnode_get_gpiod_from_child(dev,"led",&child->fwnode,\
		...
		dev_info(dev,"before gpiod_direction_output \n");
		...
		dev_info(dev,"before device_create_with_groups\n");
...
{
	{.compatible = "rvc,demo_leds_rasp3bplus_driver"},
	{ }
};
...
struct platform_driver gpiosysfs_platform_driver = 
{
	...
	.driver = {
	       .name = "demo_leds_rasp3bplus_driver",
...
{
	gpio_drv_data.class_gpio = class_create(THIS_MODULE,"rasp3bplus_demo_leds");
